...
collection.find({}).count().then(async amount => {
  ...
  fs.open('latestmsg.txt', 'r',async function (err, k) {
   if (err) {
      return console.error(err);
   }
    await db.get("__custom").then(value => {
  if(!value) db.set("__custom", 0).then(() => {});
    let numberic = 0
    if(parseInt(value) + 1 == pages[pages.length - 1].page) {
      return
    } else numberic = pages[parseInt(value)+1];
    console.log("numberic " + numberic);
