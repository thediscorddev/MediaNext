...
exports.run = async(client, message, args, clients) => {
 ...
 if (message.member.permissions.has('MANAGE_GUILD') || message.author.id == process.env['OWNERID']) {
  ...
  collection.find({id:user}).toArray(function(err, docs) {
    ...
    .setTitle(`successfully add ${parseInt(args[1])} crystal ${client.users.cache.find(u => u.id === user).tag}'s balance`)
    ...
    if(docs.length == 0) {
       collection.insertMany([{"id":user,"mpd":0,"shard":0,"crystal":parseInt(args[1]),"premium":0,"seasons":0}], function(err, result) {
        embed.addField('now:','he own:',false)
        embed.addField(`$mpd:`,`0`,true)
        embed.addField(`Shards:`,`0`,true)
        embed.addField(`Crystal:`,`${parseInt(args[1])}`,true)
