...
exports.run = async(client, message, args, clients) => {
 ...
 if (message.member.permissions.has('MANAGE_GUILD') || message.author.id == process.env['OWNERID']) {
  ...
  collection.find({id:user}).toArray(function(err, docs) {
    ...
    .setTitle(`successfully add ${parseInt(args[1])} mpd ${client.users.cache.find(u => u.id === user).tag}'s balance`)
    .setThumbnail(`https://cdn.discordapp.com/avatars/${user}/${client.users.cache.find(u => u.id === user).avatar}.webp`)
    if(docs.length == 1) {
      docs.forEach(i => {
        collection.updateOne({id:user},{$set:{"mpd":i.mpd + parseInt(args[1])}})
        embed.addField('now:','he own:',false)
        embed.addField(`$mpd:`,`${i.mpd + parseInt(args[1])}`,true)
    ...
    if(docs.length == 0) {
       collection.insertMany([{"id":user,"mpd":parseInt(args[1]),"shard":0,"crystal":0,"premium":0,"seasons":0}], function(err, result) {
        embed.addField('now:','he own:',false)
        embed.addField(`$mpd:`,`${parseInt(args[1])}`,true)
...
exports.desc = async() => {
  return `feel poor?(ok don't), or just want to acting rich? (then don't use this command lel), for staff only. (ofc)`;
}