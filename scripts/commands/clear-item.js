...
exports.run = async(client, message, args, clients) => {
  ...
  const collectionsa = clients.db("Leveling").collection('inv');
     ...
     if(!member) return message.reply(`\`m!clear-item <user ping /id> \``)
collectionsa.find({id:member.user.id}).toArray(function (err,inv) {
  if(inv.length == 0) return message.reply(`no user matched query selector`);
                 ...
                 collectionsa.updateOne({id:member.user.id},{$set:{'items':[]}},function(err,results) {
            ...
            .setTitle(`successfully clear ${member.user.username}'s item`);
