...
exports.run = async(client, message, args, clients) => {
 ...
 if (message.member.permissions.has('MANAGE_GUILD') || message.author.id == process.env['OWNERID']) {
  ...
  collection.find({id:user}).toArray(function(err, docs) {
    ...
    .setTitle(`successfully remove ${parseInt(args[1])} mpd ${client.users.cache.find(u => u.id === user).tag}'s balance`)
    ...
    if(docs.length == 0) {
       collection.insertMany([{"id":user,"mpd":0 - parseInt(args[1]),"shard":0,"crystal":0,"premium":0,"seasons":0}], function(err, result) {
        embed.addField('now:','he own:',false)
        embed.addField(`$mpd:`,`${0 - parseInt(args[1])}`,true)
