...
exports.run = async(client, message, args, clients) => {
 ...
 if (message.member.permissions.has('MANAGE_GUILD') || message.author.id == process.env['OWNERID']) {
   if(!args[0] || !args[1]) return message.reply(`invaild usage: command + item id + name`);
...
if(docs.length == 0) return message.reply(`item isn't exist in the database!`);
    ...
    collection.updateOne({"id":args[0]},{$set:{"displayname":args.slice(1).join(' ')}}, function(err, result) {
        embed.setTitle('successfully edit item\'s name')
