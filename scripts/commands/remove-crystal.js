...
exports.run = async(client, message, args, clients) => {
 ...
 if (message.member.permissions.has('MANAGE_GUILD') || message.author.id == process.env['OWNERID']) {
  ...
  collection.find({id:user}).toArray(function(err, docs) {
    ...
    .setTitle(`successfully remove ${parseInt(args[1])} crystal ${client.users.cache.find(u => u.id === user).tag}'s balance`)
    .setThumbnail(`https://cdn.discordapp.com/avatars/${user}/${client.users.cache.find(u => u.id === user).avatar}.webp`)
    if(docs.length == 1) {
      docs.forEach(i => {
        collection.updateOne({id:user},{$set:{"crystal":i.crystal - parseInt(args[1])}})
        embed.addField('now:','he own:',false)
        embed.addField(`$mpd:`,`${i.mpd}`,true)
        embed.addField(`Shards:`,`${i.shard}`,true)
        embed.addField(`Crystal:`,`${i.crystal - parseInt(args[1])}`,true)
    ...
    if(docs.length == 0) {
       collection.insertMany([{"id":user,"mpd":0,"shard":0,"crystal":0-parseInt(args[1]),"premium":0,"seasons":0}], function(err, result) {
        embed.addField('now:','he own:',false)
        embed.addField(`$mpd:`,`0`,true)
        embed.addField(`Shards:`,`0`,true)
        embed.addField(`Crystal:`,`${0-parseInt(args[1])}`,true)
