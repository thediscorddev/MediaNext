...
exports.run = async(client, message, args, clients) => {
 ...
 if (message.member.permissions.has('MANAGE_GUILD') || message.author.id == process.env['OWNERID']) {
   if(!args[0]) return message.reply(`invaild usage: command + user id`);
...
if(docs.length == 0) return message.reply(`user isn't exist in the database!`);
    ...
    collection.updateOne({"ping":args[0]},{$set:{"level":0,"xp":0}}, function(err, result) {
        embed.setTitle(`successfully reset user level`)
